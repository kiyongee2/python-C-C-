...
def select_all():
    # try:
    with sqlite3.connect("c:/py/testdb.db") as conn:
...
def insert():
    try:
        with sqlite3.connect("c:/pydb/testdb.db") as conn:
            cur = conn.cursor()
            sql = "INSERT INTO employee VALUES ('e1003', '우상혁', 3500000)"
...
def delete():
    try:
        with sqlite3.connect("c:/pydb/testdb.db") as conn:
            cur = conn.cursor()
            sql = "DELETE FROM employee WHERE id = 'e1003'"
