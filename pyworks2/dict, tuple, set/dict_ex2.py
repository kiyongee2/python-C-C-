# 딕셔너리를 이용한 성적 관리
# 여러개의 값을 저장할 수 있고, 키(key)와 값(value)으로 대응된 구조
# 학생의 성적 통계
student_list = [
    {"name": "이대한", "kor": 80, "eng": 80, "math": 75},
    {"name": "박민국", "kor": 90, "eng": 75, "math": 60},
    {"name": "최지능", "kor": 95, "eng": 98, "math": 90}
]
...
# 개인별 총점과 평균
# 소수점 자리 처리 - 콜론(:)과 점(.)찍고 자리수와 f(float) 사용
print("===== 개인별 총점과 평균 =====")
print(" 이름  총점 평균")
for st in student_list:
    total = st["kor"] + st["eng"] + st["math"]  #총점
    avg = total / 3  #평균
    print(f"{st["name"]} {total} {avg:.1f}")

# 과목별 총점과 평균
sum_subj = [0, 0, 0]  #국어총점, 수학총점을 저장
avg_subj = [0.0, 0.0, 0.0] #국어 평균, 수학 평균

# 총점 계산
for st in student_list:
    sum_subj[0] = sum_subj[0] + st["kor"]
    sum_subj[1] = sum_subj[1] + st["eng"]
    sum_subj[2] = sum_subj[2] + st["eng"]
